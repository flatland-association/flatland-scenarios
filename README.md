...
| Version                                                                                             | Description                                                                                                                                                                                                                                               |
|-----------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [FLATLAND_BENCHMARK_EPISODES_FOLDER_v7.zip](trajectories/FLATLAND_BENCHMARK_EPISODES_FOLDER_v7.zip) | Old recordings could no longer be reused this time, so the affected episodes were regenerated directly. Unlike v6, a similar shift trick was tried and tested here too, moving a train's stop one step earlier so it would not enter the next cell - but for these episodes that earlier step is often itself a real steering decision, so overwriting it sent trains down the wrong track and produced far more differences than simply regenerating did. [pr](https://github.com/flatland-association/flatland-rl/issues/178)                                                                             |
| [FLATLAND_BENCHMARK_EPISODES_FOLDER_v6.zip](trajectories/FLATLAND_BENCHMARK_EPISODES_FOLDER_v6.zip) | Old recordings could still be reused by translating their actions to the new timing, avoiding a full regeneration. v6 can be derived from v5 by a simple shift policy that, for each train from the moment it departs, replays every recorded action one step later than when it was originally taken, so that the action lines up with a train just entering a cell rather than about to leave it. [pr](https://github.com/flatland-association/flatland-rl/pull/501)                                                                    |
